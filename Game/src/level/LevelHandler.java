...
public LevelHandler(Window w) {
	...
	getTileImage();
	mapTileNumsquare=new int[32][48]; 
	mapTileNumflat=new int[32][48];
	mapTileNumback=new int[32][48];
	this.w = w;
	this.loadMap();
	Random r = new Random();
	this.seed = r.nextInt();
	this.player = new Player(w, 48, -16, 72, 48,"/map/standright.png");
	this.loadtile();
}
public void getTileImage() {
		...
		tilesquare[7].imagename="/tiles/floor7.png";
...
public void Render(Graphics g) {
	...
	if (this.player != null)	this.player.Render(g);
	g.translate(this.CameraX, this.CameraY);
}
public void loadtile() {//
	...
	while(col<32&&row<48) {
		...
		if(tileNumsquare>0)this.items.add(new Platform(ObjectIDS.Platform, x,748+y-48*46 ,48,48,tilesquare[tileNumsquare].imagename));
		if(tileNumflat>0)this.items.add(new Platform(ObjectIDS.Platform, x,748-48*46+y ,48,24,tilesflat[tileNumflat].imagename));
		if(tileNumback>0)this.items.add(new Platform(ObjectIDS.back, x,748-48*46+24+y ,48,48,tilesback[tileNumback].imagename));
		col++;
		x+=48;
		if(col==32) {
			col=0;
